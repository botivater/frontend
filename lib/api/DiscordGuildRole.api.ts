...
export const useAllDiscordGuildRoles = (guildId?: number) => {
    ...
    const { error, data } = useSWR<DiscordGuildRoleResponse>(token && guildId ? [`${configProvider.getApiEndpoint()}/v1/discord/guild-role?guildId=${guildId}`, token] : null, fetchWithToken);
