...
export const useAllMessages = (dataDto: {
  ...
  let { guildChannelId, guildMemberId, isRemovedOnDiscord, limit, offset } =
    dataDto
  ...
  queryParams.append(
    'guildChannelId',
    guildChannelId ? guildChannelId.toString() : ''
  )
  queryParams.append(
    'guildMemberId',
    guildMemberId ? guildMemberId.toString() : ''
  )
  ...
  const { error, data } = useSWR<AllMessagesResponse>(
    token && (guildChannelId || guildMemberId)
      ? [
          `${configProvider.getApiEndpoint()}/v1/message?${queryParams.toString()}`,
          token,
        ]
      : null,
    fetchWithToken
