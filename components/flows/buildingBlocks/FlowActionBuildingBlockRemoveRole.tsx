...
const FlowActionBuildingBlockRemoveRole: React.FC<Props> = ({ children, index, value, setValue }) => {
    ...
    return (
        <div className='col-span-full grid grid-cols-1 sm:grid-cols-2 gap-4'>
            <div>
                <label htmlFor={`buildingBlockRemoveRole${index}`} className='block font-bold'>Role:</label>
                <select name={`buildingBlockRemoveRole${index}`} id={`buildingBlockRemoveRole${index}`} className='w-full rounded-r-md bg-black bg-opacity-30 border-none' value={roleId} onChange={(e) => setRoleId(e.currentTarget.value)}>
