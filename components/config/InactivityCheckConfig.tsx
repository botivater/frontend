...
export const InactivityCheckConfig: React.FC<{
  inactivityCheckConfig: InactivityCheckConfig
  setInactivityCheckConfig: (
    inactivityCheckConfig: InactivityCheckConfig
  ) => void
}> = ({ inactivityCheckConfig, setInactivityCheckConfig }) => {
  const { guildId } = useContext(AppContext)!

  const {
    error: allGuildRolesError,
    data: allGuildRoles,
    isLoading: isAllGuildRolesLoading,
  } = useAllDiscordGuildRoles(guildId)
  ...
  return (
    <>
      <div>
        <label htmlFor="inactiveRoleId" className="block font-bold">
          Inactive role:
        </label>
        <select
          name="inactiveRoleId"
          ...
          value={inactivityCheckConfig.inactiveRoleId}
          onChange={(e) => {
            setInactiveRoleId(e.target.value)
          }}
        >
          ...
          {allGuildRoles &&
            allGuildRoles
              .sort(Sorting.sortDiscordGuildRolesByNameAsc)
              .map((guildRole) => (
                <option
                  key={guildRole.id}
                  value={guildRole.id}
      ...
      <div>
        <label htmlFor="activeRoleId" className="block font-bold">
          Active role:
        </label>
        <select
          name="activeRoleId"
          id="activeRoleId"
          className="w-full rounded-md border-none bg-black bg-opacity-30"
          value={inactivityCheckConfig.activeRoleId}
          onChange={(e) => {
      ...
      <div>
        <label htmlFor="inactiveUserSeconds" className="block font-bold">
          Format:
        </label>
        <input
          name="inactiveUserSeconds"
          id="inactiveUserSeconds"
          type={'number'}
          className="w-full rounded-md border-none bg-black bg-opacity-30"
          value={inactivityCheckConfig.inactiveUserSeconds}
          onChange={(e) => {
            setInactiveUserSeconds(parseInt(e.target.value))
          }}
          placeholder={'7776000'}
        />
        <small className="block">
          Enter the inactivity timeout in seconds.
        </small>
      </div>
    </>
  )
