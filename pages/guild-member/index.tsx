...
const GuildMemberPage: NextPage = () => {
  ...
  const data = useMemo<GuildMember[]>(() => allGuildMembers || [], [allGuildMembers]);
  const columns = useMemo<Column<GuildMember>[]>(() => [
    ...
    },
    {
      ...
      Cell: (props: { value: string }) => {
        return <span>{new Date(props.value).toLocaleString('nl', { dateStyle: 'long', timeStyle: 'medium' })}</span>;
      }
    },
    {
      Header: 'Updated at',
        ...
        {user &&
          <div className='max-w-6xl mx-auto px-4 flex flex-col gap-4'>
            <div>
              ...
              <p className='text-white text-opacity-30'>An overview of the members.</p>
            </div>
            <div>
                          ...
                          <tr key={key} {...headerGroupProps}>
                            {
                              headerGroup.headers.map(column => {
                                ...
                                return (
                                  <th key={key} {...headerProps} className="whitespace-nowrap p-2">
                  ...
                  <tbody {...table.getTableBodyProps()}>
                    {
                      table.rows.map(row => {
                        ...
                        return (
                          <tr key={key} {...rowProps} className="even:bg-gray-500 odd:bg-gray-600 hover:bg-gray-900 duration-75 transition-all">
                            {
                              row.cells.map(cell => {
                                const { key, ...cellProps } = cell.getCellProps();

                                return (
                                  <td key={key} {...cellProps} className="whitespace-nowrap p-2">
                                    {cell.render('Cell')}
                                  </td>
                                )
                              })
